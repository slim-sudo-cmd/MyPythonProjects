...
class RegistrationAndLogin:
   def __init__(self,UserDataDetails,Array):
      ...
      self.Array = Array
      self.HomeInstance = Home(UserDataDetails,Array)
   ...
   @classmethod
   def LogInPanelAction(cls,UserDataDetails,Array,AdminInfo):
      cls.HomeInstance = Home(UserDataDetails,Array)
      ...
      if cls.LoginPanel == 1:return cls.UserAccountRegistration(UserDataDetails,Array)
...
class Home:
   ...
   def HomePageIntro(cls,UserDataDetails,Array):
      ...
      print("________ Home Page ________")
      ...
      if cls.HomeIn == 2:return cls.TrialInstance.TrialExploreIntro(UserDataDetails,Array)
...
class AdminPanel:
   def __init__(self,AdminInfo,StockInventory):
      ...
      self.StockInventory = {
                    "Bakery":{
                              ...
                              "Mandazi/Mahamri":{"Price":0,"Available Stock":0},
                    ...
                    "Dry Pantry & Grains":{
                               "Pembe 2kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Soko 2kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Ajab 2kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Amaize 2kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Raha 2kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Pisha 2kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Jogoo 2kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Pembe 1kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Soko 1kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Ajab 1kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Amaize 1kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Raha 1kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Pisha 1kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Jogoo 1kg Maize Flour":{"Price":0,"Available Stock":0},
                               "Pembe 2kg Wheat Flour":{"Price":0,"Available Stock":0},
                               "Soko 2kg Wheat Flour":{"Price":0,"Available Stock":0},
                               "Ajab 2kg Wheat Flour":{"Price":0,"Available Stock":0},
                               "Raha 2kg Wheat Flour":{"Price":0,"Available Stock":0},
                               "Pembe 1kg Wheat Flour":{"Price":0,"Available Stock":0},
                               "Soko 1kg Wheat Flour":{"Price":0,"Available Stock":0},
                               "Ajab 1kg Wheat Flour":{"Price":0,"Available Stock":0},
                               "Raha 1kg Wheat Flour":{"Price":0,"Available Stock":0},
                               "Soko 2kg Uji Sour Wimbi":{"Price":0,"Available Stock":0},
                               "Familia 2kg Uji Sour":{"Price":0,"Available Stock":0},
                               "Familia 2kg Pure Wimbi":{"Price":0,"Available Stock":0},
                               "Soko 2kg Pure Uji Wimbi":{"Price":0,"Available Stock":0},
                               "Familia 1kg Uji Sour":{"Price":0,"Available Stock":0},
                               "Familia 1kg Uji Pure Wimbi":{"Price":0,"Available Stock":0},
                               "Soko 1kg Uji Sour":{"Price":0,"Available Stock":0},
                               "Soko 1kg Pure Uji Wimbi":{"Price":0,"Available Stock":0},
                               ...
                               "Salt 1/2":{"Price":0,"Available Stock":0},
   ...
   def StockActionIntro(cls,UserDataDetails,Array,AdminInfo,StockInventory):
      cls.RegistrationInstance = RegistrationAndLogin(UserDataDetails,Array)
      ...
      if cls.StockAction == 1:return cls.StockCategories(StockInventory)
      elif cls.StockAction == 2:return cls.FinanceStatus()
      elif cls.StockAction == 3:return cls.RegistrationInstance.UserAccountRegistration(UserDataDetails,Array)
      else:return None


   """ Display Category Elementa"""
   @classmethod
   def AdminStock(cls,StockAvailableIn):
      print("\n______ Stock In Shop _____\n")
      for i,(key,value) in enumerate(StockAvailableIn.items()):print(f">> {i}. {key}:{value}")
      cls.StockIntro = int(cls.GetAdminInputs("\n>>> Continue with ;\n>> 1: Add Stock\n>> 2: Remove Stock\n>> 3: Check All Stock Available\n>> 4: Go Back\n>>> Admin Option :",lambda x: x.isdigit() and int(x) in [1,2,3,4,5],">>> Select a valid Input."))
      if cls.StockIntro == 1:return cls.StockCategories()


   """ First display Stock category """
   @classmethod
   def StockCategories(cls,StockInventory):
      print("\n______ Stock Per Category ______\n\n>>> All categories of Stock Commodities.\n")
      for i,category in enumerate(cls.StockInventory.keys()):print(f">> {i}: {category}")
      cls.CategorySelect = int(cls.GetAdminInputs("\nContinue with;\n>> 1: Add Stock category\n>> 2: Remove Stock category\n>> 3: Go Back\n>>> Option :",lambda x:x.isdigit() and int(x) in [1,2,3],">>> Select a valid Input."))
      if cls.CategorySelect == 1:return cls.AddStockCategory(StockInventory)
      elif cls.CategorySelect == 2:return cls.RemoveStockCategory(StockInventory)
      elif cls.CategorySelect == 3:return cls.StockActionIntro(UserDataDetails,Array,AdminInfo,StockInventory)
      else:return None


   def UpdateStockCommodities(self,Item,Quantity):
      self.StockAvailableIn[Item] = self.StockAvailableIn.get(Item,0) + int(Quantity)

   def SellStockCommodities(self,Item):
      if Item in self.StockAvailableIn and self.StockAvailableIn[Item] > 0:
         self.StockAvailableIn[Item] -=1
         print(f">>> Sold :{Item}.Remaining :{self.StockAvailableIn[Item]}.")



   @staticmethod
   def AppendingStockIn(Prompt,ConditionFunc,ErrMsg,Stock,key):
      while not ConditionFunc(val := input(Prompt)):
         print(f">>> {val} is not allowed.\n{ErrMsg}")
      Stock[key] = val
      return val
   ...
   @classmethod
   def AddStockCategory(cls,StockInventory):
      print("\n______ Append Stock ______\n\n>>> Current Stock Categories include:\n")
      for i,key in enumerate(StockInventory.key()):print(">> {i}: {key}")
...
class StockManagement:
   def __init__(self,StockAvailable,UserDataDetails,UserAdmin):
      self.StockAvailable = StockAvailable
      self.UserDataDdetails = UserDataDetails
      self.UserAdmin = UserAdmin

   @staticmethod
   def AppendItemKeys(Prompt,ConditionFunc,ErrMsg,StockAvailable,key):
      pass


   @staticmethod
   def AppendItemValue(Prompt,ConditionFunc,ErrMsg,StockAvailable,value):
      pass


   @classmethod
   def StockItem(cls,StockAvailable):
      pass
...
if __name__ == '__main__':
   ...
   RegistrationAndLogin.LogInPanelAction(UserDataDetails,Array,AdminInfo)
