...
class ArrayActivity:
   ...
   def ColumnOperations():pass
   def RowOperations():pass


   @staticmethod
   def MatrixOn(row,cols,dtype=np.float64):
      Matrix = np.zeros((row,cols),dtype=dtype)
      for i in range(row):
         for j in range(cols):
            val = ArrayActivity.GetArrayInputs(f"Set Values [{i},{j}]:",lambda x:x.replace('.','',1).isdigit(),"Invalid input")
            Matrix[i,j] = float(val)
      return Matrix

   @classmethod
   def CreatingMatrix(cls):
      cls.MatrixRowNo = int(GetArrayInputs("\n>>>    Array Creation   >>>\n• You will create an Array of own\n>> To continue select Order of your array\n\n>>> Enter No of Rows:",lambda x: x.isdigit() and int(x) <= 5,">>> Please enter a valid row range."))
      cls.MatrixColumnNo = int(GetArrayInputs("\n>>> Enter No of Columns :",lambda x: x.isdigit() and int(x) <=5,">>> Please enter a valid column range."))
      cls.ActiveMatrix = cls.MatrixOn(cls.MatrixRowNo,cls.MatrixColumnNo)
      print()
